...
class ProductController extends Controller
{
    ...
    public function getAllPaginated(Request $request)
    {
        ...
        try {
            $productCategories = $this->productRepository->getAllPaginated(
                $request['search'] ?? null,
                $request['product_category_id'] ?? null,
                $request['row_per_page']
            );

            return ResponseHelper::jsonResponse(true, 'Data Produk Berhasil Diambil', PaginateResource::make($productCategories, ProductResource::class), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
