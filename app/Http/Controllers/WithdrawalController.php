...
class WithdrawalController extends Controller implements HasMiddleware
{
    ...
    {
        try {
            $witdrawal = $this->withdrawalRepository->getById($id);

            if (!$witdrawal) {
                return ResponseHelper::jsonResponse(false, 'Data Withdrawal Tidak ditemukan', null, 404);
            }

            return ResponseHelper::jsonResponse(true, 'Data Withdrawal berhasil di ambil', new WithdrawalResource($witdrawal), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
    ...
    public function approve(WithdrawalApproveRequest $request, string $id)
    {
        ...
        try {
            ...
            return ResponseHelper::jsonResponse(true, 'Data Withdrawal berhasil Disetujui', new WithdrawalResource($witdrawal), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
